...
public class FirstBid extends TableState {
	...
	public void Auto(Table tab) {
		...
		for (int i = 0; i < tab.countActivePlayers(); i++) {
			try {
				...
				System.out.println("Bid: " + CurrPlayer + " BET: " + tab.getSystemPlayer(CurrPlayer).getPlayerBet()
						+ " cards: " + tab.getSystemPlayer(CurrPlayer).countCards());
				Decode(getAction());
	...
	/*
	 * public void Check() { // TODO Auto-generated method stub
	 * if(tab.getSystemPlayer(CurrPlayer).getPlayerStatus()!=PlayerStatus.
	 * max_bet_bb) if(tab.getBets().get(player)<tab.getHighestBet()){
	 * tab.getPlayer(player).notify(
	 * "Nie można podtrzymać stawki - została przebita."); Decode(getAction());
	 * } }
	 */
	...
	public void Raise(Integer betincr) {
		...
		if ((status == PlayerStatus.max_bet_nbb)) {
			...
			if ((lim == Limit.no_limit)
					|| (lim == Limit.fixed_limit && betincr <= maxRaiseValue && raiseCounter < maxRaiseCount)
					|| (lim == Limit.pot_limit && betincr <= pot)) {
				try {
